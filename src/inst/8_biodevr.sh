...
case "$choice" in
  y|Y ) # notify start
        ...
        sudo apt-get install default-jre default-jdk curl openssl libxml2-dev libssl-dev libcurl4-openssl-dev libnlopt-dev libgeos-dev texlive-latex-extra libfontconfig1-dev libharfbuzz-dev libfribidi-dev -y
        ...
        case "$choice" in
          y|Y ) # notify start
                ...
                wget -q https://download1.rstudio.org/electron/jammy/amd64/rstudio-2022.12.0-353-amd64.deb && echo '"RStudio" deb package is downloaded.' && sleep 1 #_to_be_updated
