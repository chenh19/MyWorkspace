...
# notify start
sudo echo ""
echo -e "${TEXT_YELLOW}Configuring KDE plasma system settings...${TEXT_RESET}" && sleep 1
...
kwriteconfig5 --file ~/.config/mimeapps.list --group 'Added Associations' --key 'text/x-r-markdown' "org.kde.kwrite.desktop;rstudio.desktop;libreoffice-writer.desktop;okularApplication_txt.desktop;"
...
kwriteconfig5 --file ~/.config/mimeapps.list --group 'Default Applications' --key 'text/x-r-markdown' "org.kde.kwrite.desktop;"
...
cat /proc/bus/input/devices | grep -i syna | grep "Touchpad"> ~/.config/touchpadxlibinputrc
...
## Remove Hibernate
if grep -q "#AllowHibernation=yes" /etc/systemd/sleep.conf ; then sudo sudo sed -i 's+#AllowHibernation=yes+AllowHibernation=no+g' /etc/systemd/sleep.conf ; fi
