...
printf '%s\n' '[Desktop Entry]' 'Type=Service' 'ServiceTypes=KonqPopupMenu/Plugin' 'MimeType=image/jpeg;image/png;' 'Icon=image-png' 'Actions=RotateLeft' 'X-KDE-Priority=TopLevel' 'X-KDE-StartupNotify=false' '' '[Desktop Action RotateLeft]' 'Icon=object-rotate-left' 'Exec=bash -lc '"'"'for f in "$@"; do tmp="${f}.tmp"; mv -- "$f" "$tmp" && convert -rotate 270 -- "$tmp" "$f" && rm -f -- "$tmp" || { rc=$?; [ -e "$tmp" ] && mv -- "$tmp" "$f"; exit $rc; }; done'"'"' _ %F' 'Name=Rotate Left' 'Name[en_US]=Rotate Left' 'Name[zh_CN]=向左旋转' > ~/.local/share/kio/servicemenus/rotate_left.desktop
printf '%s\n' '[Desktop Entry]' 'Type=Service' 'ServiceTypes=KonqPopupMenu/Plugin' 'MimeType=image/jpeg;image/png;' 'Icon=image-png' 'Actions=RotateRight' 'X-KDE-Priority=TopLevel' 'X-KDE-StartupNotify=false' '' '[Desktop Action RotateRight]' 'Icon=object-rotate-right' 'Exec=bash -lc '"'"'for f in "$@"; do tmp="${f}.tmp"; mv -- "$f" "$tmp" && convert -rotate 90 -- "$tmp" "$f" && rm -f -- "$tmp" || { rc=$?; [ -e "$tmp" ] && mv -- "$tmp" "$f"; exit $rc; }; done'"'"' _ %F' 'Name=Rotate Right' 'Name[en_US]=Rotate Right' 'Name[zh_CN]=向右旋转' > ~/.local/share/kio/servicemenus/rotate_right.desktop
...
chmod +x ~/.config/background/wallpaper.sh
chmod +x ~/.config/background/sddm.sh
